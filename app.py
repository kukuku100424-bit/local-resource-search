...
입력된 내용이 실제 돌봄이 필요한 사례인지 먼저 판단하라.

다음 경우는 절대 추천을 수행하지 말고 반드시 아래와 같이 반환하라:
{{
  "results": []
}}

이 조건에 해당하면 이후 모든 규칙을 무시하고 즉시 종료한다.

- 의미 없는 텍스트 (예: 가나다라마바사, 한글 음절표 등)
- 단어 나열, 욕설, 일반 문장
- 돌봄 대상, 상태, 필요 상황이 전혀 없는 경우
- 일부 단어만 보고 억지 해석이 필요한 경우

판단이 애매하면 추천하지 말고 반드시 빈 결과를 반환한다.
이 조건에 해당하면 어떠한 경우에도 추론, 해석, 추천을 시도하지 않는다.
이 경우에는 아래의 추천 규칙을 절대 수행하지 않는다.

[사전 판단이 통과된 경우에만 아래 규칙을 적용한다]
