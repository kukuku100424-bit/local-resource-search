...
def desc():
    ...
    if request.method == "POST" and do_search:
        ...
        if "치매" in q_norm and any(k in q_norm for k in ["가스레인지", "가스렌지", "가스불", "화기", "불관리", "가스차단", "화재위험", "불을켜놓", "가스잠그", "냄비태우", "냄비를태우"]):
            extra_aliases += ["일상생활돌봄", "스마트돌봄", "응급안전안심서비스", "화재감지기", "가스타이머차단기", "안전확인"]
...
5. 특히 아래 표현은 적극 반영한다.
   ...
   - 치매·인지저하로 인해 가스불을 켜놓고 잊어버리거나 냄비를 태우는 등 인지 문제로 인한 화재·가스 위험 → 일상생활돌봄-스마트돌봄-응급안전안심서비스(가스타이머차단기, 화재감지기 등)를 우선 검토한다
   - 가스배관 노후, 밸브 고장 등 물리적 시설 결함이 명시된 화재·가스 안전 문제 → 주거복지-주거공간개선-주거안전지원을 검토한다
...
def expand_query_aliases(query: str):
    ...
    if "치매" in q_norm and any(k in q_norm for k in ["가스레인지", "가스렌지", "가스불", "화기", "불관리", "가스차단", "화재위험", "불을켜놓", "가스잠그", "냄비태우", "냄비를태우"]):
        aliases.extend(["일상생활돌봄", "스마트돌봄", "응급안전안심서비스", "화재감지기", "가스타이머차단기", "안전확인"])
