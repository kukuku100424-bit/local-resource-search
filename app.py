...
5. 특히 아래 표현은 적극 반영한다.
   ...
   - 화재·가스 안전 문제 판단 기준 (매우 중요):
     · 치매·인지저하로 인해 가스불을 켜놓고 잊어버리거나 냄비를 태우는 등 "인지 문제로 인한" 화재·가스 위험 → 반드시 "일상생활돌봄-스마트돌봄-기타"를 추천한다 (이 소분류는 응급호출기·화재감지기·가스타이머차단기 등 스마트 안전기기 설치를 포괄하는 항목이다)
     · 이 경우 "주거안전지원(문턱제거, 안전손잡이 설치 등)"은 낙상 방지용 물리적 시설 개선 항목이므로, 낙상·이동불편·문턱·계단 등 별도의 언급이 없는 한 추천하지 않는다
     · 가스배관 노후, 밸브 고장 등 물리적 시설 결함이 직접 언급된 경우에만 주거복지-주거공간개선-주거안전지원을 검토한다
     · 인지 문제로 인한 위험과 물리적 시설 결함이 함께 언급되면 두 서비스를 함께 추천할 수 있다
