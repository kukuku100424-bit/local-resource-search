...
[돌봄 사례 여부 판단]
아래 3가지가 모두 있을 때만 추천한다.
1. 돌봄 대상자
2. 일상생활·건강·안전·정서·주거 중 구체적 어려움
3. 서비스나 도움 필요 상황

책 내용, 일반 설명, 단순 정보, OCR로 찍힌 무관한 문장은 추천하지 않는다.
애매하면 반드시 {{"results": []}} 만 반환한다.
...
10. 사용자의 직접 욕구에 해당하는 서비스만 더 높은 우선순위로 배치하고, 그 외 서비스는 일반 추천으로 유지한다.
   - 사용자가 특정 영역을 명확히 요청한 경우(예: 식사, 외출 등), 다른 영역(예: 건강관리, 간호, IoT 등)은 직접 언급되지 않았다면 과도하게 확장하여 추천하지 않는다.
11. 사용자의 요청이 특정 영역(예: 식사, 이동, 주거 등)에 명확히 해당하는 경우, 다른 영역(예: 건강관리, 간호, IoT 등)은 직접적으로 언급되지 않았다면 과도하게 확장하여 추천하지 않는다.
