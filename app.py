...
def desc():
    ...
    if request.method == "POST" and do_search:
        ...
        if any(k in q_norm for k in ["주거", "집", "주택", "거주환경", "주거환경", "집수리", "노후", "곰팡이", "해충", "악취", "쓰레기"]):
...
5. 특히 아래 표현은 적극 반영한다.
   ...
   - 가스레인지, 화기관리, 불을 켜놓음, 가스차단 등 화재·가스 안전 문제 → 주거복지-주거공간개선-주거안전지원 우선 검토
...
- 건강상태·질환과 함께 '필요/원함' 표현이 쓰여도 true로 처리한다 (예: '편마비가 있어 재활 서비스 필요' → 재활 관련 서비스 direct_need=true)
...
- 입력에 '원함·희망·받고 싶어함·욕구·필요' 표현이 전혀 없이, 상태 서술(예: 편마비가 있음, 곰팡이가 있음, 자주 넘어짐)만으로 시스템이 서비스를 추론한 경우 (입력에 '필요/원함' 표현이 있으면 위 true 규칙을 우선한다)
